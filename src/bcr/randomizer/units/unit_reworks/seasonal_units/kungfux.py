...
def kungfux(stats:list[list[list]]):
    ...
    for x in range(0,3):
        unit[x][c.s.hp] = 1234 #makes for 20978 at 30, was 999
    #first form has a desperation mode
    unit[0][c.s.attack] = 1780 #makes for 30260 at 30 was 1280
    unit[0][c.s.strengthen_by] = 50
    unit[0][c.s.strengthen_at] = 10
    #second form has zkill on last hit, does that work?
    (unit[1][c.s.attack],unit[1][c.s.multi_damage_2],unit[1][c.s.multi_damage_3]) = (372,214,1336) #makes for (6324,3638,22712) at 30, was (282,140,986)
    unit[1][c.s.multi_has_ability_1] = 0
    unit[1][c.s.multi_has_ability_2] = 0
    unit[1][c.s.multi_has_ability_3] = 1
    unit[1][c.s.zombie_killer] = 1
    #third form has wave on the first hit
    (unit[2][c.s.attack],unit[2][c.s.multi_damage_2],unit[2][c.s.multi_damage_3]) = (427,234,1546) #makes for (7259,3978,26282) at 30, was (370,184,1294)
